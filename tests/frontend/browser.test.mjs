...
test("official sandbox boots and preserves critical workflows", {
	timeout: 45000,
}, async () => {
	...
	try {
		...
		const source = await readFile(
			resolve(ROOT, "pages/style-manager/index.html"),
			"utf8",
		);
		...
		await page.setContent(
			'<iframe id="app" title="plugin" sandbox="allow-scripts allow-forms allow-downloads"></iframe>',
		);
		...
		assert.equal(
			await frame.locator("#curSid").getAttribute("title"),
			"a:test",
		);
		assert.equal(
			await frame
		...
		assert.equal(
			await frame.locator(".session-item").first().getAttribute("title"),
			"a:test",
		);

		const scrollState = await frame.evaluate(() => ({
			workspace: {
				client: document.querySelector("#ws").clientHeight,
				scroll: document.querySelector("#ws").scrollHeight,
			},
			sessionList: {
				client: document.querySelector("#sessList").clientHeight,
				scroll: document.querySelector("#sessList").scrollHeight,
			},
		}));
		assert.ok(
			scrollState.workspace.scroll > scrollState.workspace.client,
			"workspace should be vertically scrollable",
		);
		assert.ok(
			scrollState.sessionList.scroll >= scrollState.sessionList.client,
			"session list should retain its scroll container",
		);
		const workspaceScrollTop = await frame
			.locator("#ws")
			.evaluate((element) => {
		...
		assert.ok(
			...
			"workspace scrollTop should respond to scrolling",
		);
		await frame.getByRole("button", { name: "去重本会话" }).click();
		assert.match(await frame.locator("#mBody").textContent(), /安全重复/);
		assert.equal(
			await frame.locator("#mOk").getAttribute("class"),
			"btn btn-danger",
		);
		await frame.locator("#mOk").click();
		await frame.waitForFunction(
			() => window.__MOCK.universal["a:test"].length === 1,
		);
		assert.deepEqual(await frame.evaluate(() => window.__DEDUP_CALLS), [
			"a:test",
		]);
		...
		assert.equal(
			await frame.locator("html").getAttribute("data-theme"),
			"light",
		);
		await frame.evaluate(() => window.__setTheme(true));
		assert.equal(
		...
		await frame.waitForFunction(
			() => document.activeElement?.id === "helpClose",
		);
		const helpText = await frame.locator("#helpOv").textContent();
		assert.match(helpText, /嵌套量词/);
		assert.match(helpText, /200/);
		...
		assert.equal(
			await frame.locator("#ovPortrait").textContent(),
			'通用风格：（暂无）；情境提示：其他场景→其他行为；特定层数据：<img src=x onerror="window.__XSS=1">',
		);
		const overviewNote = await frame.locator(".ov-note").first().textContent();
		assert.doesNotMatch(overviewNote, /强约束/);
		assert.match(overviewNote, /不得覆盖原有身份、安全要求或任务约束/);
		assert.deepEqual(
			await frame
				.locator(".donut circle")
				.evaluateAll((nodes) =>
		...
		assert.equal(
			await specificRegex.evaluate((input) =>
		...
		await frame
			.getByPlaceholder("风格描述，如：语气活泼、爱用短句")
			.first()
			.fill("新通用");
		...
		await frame.waitForFunction(
			() => !document.querySelector("#dirtyBanner").classList.contains("show"),
		);
		assert.deepEqual(
		...
		assert.deepEqual(await frame.evaluate(() => window.__LAYER_CALLS), [
			"universal",
			"contextual",
		]);
		...
		assert.equal(
			await frame.evaluate(() => document.querySelector("#overlay").open),
			false,
		);
		assert.equal(
			await frame.evaluate(() => window.__MOCK.universal["a:test"].length),
			1,
		);
		...
		await frame.getByRole("button", { name: "清空本会话" }).click();
		assert.match(
			await frame.locator("#mBody").textContent(),
			/本地未保存修改也会丢弃/,
		);
		await frame.locator("#mOk").click();
		await frame.waitForFunction(
		...
		for (const width of [320, 768, 1280]) {
			...
			const overflow = await frame.evaluate(() => ({
				...
				tabRight: document
					.querySelector('[data-tab="specific"]')
			...
			assert.ok(
				overflow.tabRight <= overflow.client,
				`tab overflow at ${width}px`,
			);
		}
