...
export const LAYERS = [
	...
	},
	{
		key: "specific",
		name: "特定",
		color: "var(--c-specific)",
		soft: "var(--c-specific-soft)",
		hint: "群内梗与释义。每次回复全量注入 content；trigger_regex 只用于更新 trigger_count / last_seen。保存时由服务端校验正则语法并提示具体条目。",
		icon: "hash",
	},
];
...
export function acceptSavedLayer(
	...
	if (
		expectedEditSeq !== null &&
		(store.editSeq?.[key] ?? 0) !== expectedEditSeq
	)
		return false;
